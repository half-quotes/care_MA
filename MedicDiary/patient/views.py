...
def predict(request):
    ...
    data = {
    'HEALTHCARE_COVERAGE':profile.healthcare_coverage,
    'HEALTHCARE_EXPENSES':profile.healthcare_expenses,
    'age':profile.age,
    'CITY':profile.city,
    ...
    urlz = 'https://maapi-production.up.railway.app/predict' 
    for x in requests.post(urlz, data=data):
        b=x
        c=str(b)
        response=int(c[54])
    k='null'
    if response==1:
        k="Medically Adhere"
    if response==0:
        k="Do not Adhere Medically"
    else:
        k=c
        
    return render(request, 'patient/predict.html',{'profile':profile,'response':k})
